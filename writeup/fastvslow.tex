...
These datacubes are then analysed using the MaNGA data analysis pipeline (DAP); the development of which is ongoing and will be described in detail in Westfall et al. (in preparation). The DAP provides measurements of flux, stellar-continuum fits, absorption- and emission-line properties and spectral index measurements. These include the mean flux, stellar velocity and stellar velocity dispersion in each `spaxel'. These measurements allow for the calculation of the stellar specific angular momentum of a galaxy (see Section~\ref{sec:mangasample}). 
...
Calculate $\lambda_R$ for these galaxies currently observed by MaNGA using the Emsellem et al. equation. This reveals $106$ ($63\%$) regular rotators and $61$ ($37\%$) non-regular rotators, split by the definition in \citet{cappellari16}. Velocity dispersion measurements in each spaxel were confirmed to be above the instrument resolution of $77~\rm{km}~\rm{s}^{-1}$. They are shown by their velocity maps in Figure~\ref{fig:evsl}. 
...
An example posterior probability distribution output by \textsc{starpy} is shown for a single galaxy in Figure 5 of \cite{smethurst15}, wherein the degeneracies of the SFH model between recent, rapid quenching and earlier, slower quenching can clearly be seen. To study the SFH across a population of galaxies, these individual posterior probability distributions are stacked in $[t_q, \tau]$ space. This is no longer inference but is a method to visualise the results across a population of galaxies (see appendix section C in \citealt{smethurst16} for a discussion on alternative methods which may be used to determine the parent population SFH). These distributions will be referred to as the population SFH densities.
